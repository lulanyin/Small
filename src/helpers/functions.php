...
if(!function_exists('getVerifyCode')){
    ...
    function getVerifyCode($name='verify_code'){
        return strtolower(\Small\lib\util\Request::getSession($name));
    }
     ...
     * @return bool
     */
    function matchVerifyCode($input, $name='verify_code'){
        if(!empty($input)){
            if(getVerifyCode($name)==strtolower($input)){
                return true;
            }else{
                $vcid = \Small\lib\util\Request::post("verify_code_id");
                if(is_numeric($vcid)){
                    return \Small\model\models\VerifyCodeModel::verify($vcid, $input);
                }
            }
        }
        return false;
    ...
    function resetVerifyCode($name='verify_code'){
        \Small\lib\util\Request::dropSession($name);
    }
}
...
function getUE($field, $html, $small=false, $width="100%", $height="400px") : string {
    $html = \Small\lib\util\Str::htmlDecode(\Small\lib\util\Str::htmlDecode($html));
    $editor_html = array();
